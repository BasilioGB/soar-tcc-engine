...
Ambientes digitais modernos produzem grande quantidade de eventos de segurança provenientes de firewalls, sistemas de detecção, ferramentas de proteção de endpoint, serviços em nuvem, aplicações corporativas e outras fontes de monitoramento. Em um Centro de Operações de Segurança (SOC), esses eventos precisam ser analisados, priorizados e tratados por analistas, muitas vezes sob pressão de tempo e com dependência de procedimentos repetitivos.

Embora a atuação humana continue essencial para tomada de decisão, investigação e julgamento contextual, parte significativa da resposta a incidentes envolve tarefas recorrentes, como enriquecimento de indicadores, consulta a serviços externos, classificação de severidade, atualização de status, registro de evidências e notificação de responsáveis. Quando essas atividades são executadas manualmente, podem ocorrer atrasos, divergências entre analistas e perda de histórico operacional.

As plataformas SOAR buscam reduzir esses problemas ao combinar orquestração, automação e resposta a incidentes. Um dos elementos centrais desse tipo de solução é o playbook, que representa uma sequência estruturada de ações a serem executadas diante de determinado cenário. Playbooks podem padronizar condutas, apoiar o tempo de resposta, reduzir variação entre analistas e permitir que integrações externas sejam utilizadas de forma coordenada.
...
\begin{tabular}{|L{2.7cm}|L{6.5cm}|L{3.8cm}|}
\hline
\textbf{Componente} & \textbf{Responsabilidade principal} & \textbf{Evidência no projeto} \\ \hline
Incidentes e artefatos & Representar casos, IOCs, tarefas, timeline, impacto e contexto operacional & \texttt{incidents/models.py} \\ \hline
Playbooks e DSL & Armazenar definições, validar estrutura, sincronizar gatilhos/filtros e registrar execuções & \texttt{playbooks/models.py}, \texttt{playbooks/dsl.py} \\ \hline
Automação e eventos & Emitir eventos, localizar gatilhos, aplicar filtros e iniciar execuções & \texttt{automation/events.py}, \texttt{automation/tasks.py}, \texttt{automation/matcher.py} \\ \hline
Runner de execução & Avaliar \texttt{when}, resolver entradas, executar ações, tratar erros e persistir resultados & \texttt{automation/runner.py} \\ \hline
Integrações & Resolver ações internas ou conectores HTTP configuráveis & \texttt{integrations/registry.py} \\ \hline
API e interface web & Validar, criar, listar e acionar playbooks e incidentes & \texttt{api/views.py}, \texttt{webui/views.py} \\ \hline
Rastreabilidade & Registrar logs, resultados por etapa, timeline e auditoria & \texttt{ExecutionLog}, \texttt{ExecutionStepResult}, \texttt{TimelineEntry}, \texttt{audit} \\ \hline
\end{tabular}
...
\begin{tabular}{|L{2.7cm}|L{5.8cm}|L{4.5cm}|}
\hline
\textbf{Critério} & \textbf{O que avalia} & \textbf{Evidência no projeto} \\ \hline
Funcionalidade & Capacidade de criar, validar e executar playbooks de incidente e artefato & DSL, modelos Playbook/Execution, endpoints e UI \\ \hline
Flexibilidade & Adaptação dos fluxos por filtros, labels, placeholders, \texttt{when}, \texttt{on\_error} e \texttt{control.branch} & Parser da DSL, runner e testes de branching \\ \hline
Integração & Capacidade de acionar ações internas e conectores HTTP/API REST & Registro de ações e integrações configuráveis \\ \hline
Rastreabilidade & Registro de execução, logs, resultados por etapa e timeline do incidente & Execution, ExecutionLog, ExecutionStepResult e TimelineEntry \\ \hline
Separação manual/automática & Diferenciação entre automação por evento e procedimento acionado pelo analista & Labels \texttt{auto-treatment} e \texttt{manual-treatment}, filtros e gatilhos \\ \hline
Testabilidade & Verificação automatizada da DSL, filtros, execução, integração e endpoints & Testes em \texttt{playbooks}, \texttt{automation}, \texttt{integrations} e \texttt{api} \\ \hline
Tempo de resposta em cenário controlado & Possibilidade de comparar fluxo manual e automático em cenário controlado & Campos de ciclo do incidente e duração por etapa de execução \\ \hline
\end{tabular}
\label{tab:criterios}
\end{table}
...
\caption{Cenários comparativos do estudo demonstrativo.}
\begin{tabular}{|L{2.1cm}|L{2.3cm}|L{2.3cm}|C{0.9cm}|L{2.7cm}|L{2.7cm}|}
\hline
\textbf{Cenário AUTO x MANUAL} & \textbf{Labels do fluxo automático} & \textbf{Labels do fluxo manual} & \textbf{Artefatos} & \textbf{Playbook automático} & \textbf{Playbook manual} \\ \hline
Phishing genérico & \texttt{phishing}, \texttt{email}, \texttt{auto-treatment} & \texttt{phishing}, \texttt{email}, \texttt{manual-treatment} & 4 & Phishing triage e enriquecimentos & Phishing manual checklist \\ \hline
Roubo de credenciais & \texttt{phishing}, \texttt{credential-compromise}, \texttt{auto-treatment} & \texttt{phishing}, \texttt{credential-compromise}, \texttt{manual-treatment} & 4 & Credential phishing containment & Credential compromise manual checklist \\ \hline
Malware por phishing & \texttt{phishing}, \texttt{malware-suspected}, \texttt{auto-treatment} & \texttt{phishing}, \texttt{malware-suspected}, \texttt{manual-treatment} & 5 & Malware phishing containment & Malware suspected manual checklist \\ \hline
Conta de e-mail comprometida & \texttt{phishing}, \texttt{mailbox-compromise}, \texttt{auto-treatment} & \texttt{phishing}, \texttt{mailbox-compromise}, \texttt{manual-treatment} & 4 & Mailbox compromise response & Mailbox compromise manual checklist \\ \hline
Infraestrutura maliciosa baseada em IOC & \texttt{ioc-malicious-infrastructure}, \texttt{auto-treatment} & \texttt{ioc-malicious-infrastructure}, \texttt{manual-treatment} & 4 & IOC malicious infrastructure response & IOC malicious infrastructure manual checklist \\ \hline
\end{tabular}
\label{tab:cenarios}
\end{table}
...
\begin{tabular}{|L{2.6cm}|L{4.1cm}|L{3.7cm}|L{3.2cm}|}
\hline
\textbf{Métrica} & \textbf{Como calcular} & \textbf{Fonte no projeto} & \textbf{Interpretação} \\ \hline
MTTA & \texttt{responded\_at - detected\_at} & \texttt{Incident.response\_delta()} & Tempo até a primeira resposta registrada \\ \hline
MTTR & \texttt{resolved\_at} ou \texttt{closed\_at - detected\_at} & \texttt{Incident.resolution\_delta()} & Tempo até resolução ou fechamento \\ \hline
Duração da automação & Soma, média ou análise dos valores de \texttt{duration\_ms} & \texttt{ExecutionStepResult} & Custo de execução do playbook \\ \hline
Tempo manual de referência & Cronometragem do procedimento manual equivalente & Cenários demonstrativos & Base comparativa para o fluxo humano \\ \hline
Ganho estimado & Tempo manual de referência menos tempo automatizado & Comparação controlada & Redução potencial de esforço operacional \\ \hline
\end{tabular}
\label{tab:metricas}
\end{table}
